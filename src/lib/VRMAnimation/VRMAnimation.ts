  ...
  public createHumanoidTracks(vrm: VRM): THREE.KeyframeTrack[] {
    ...
    for (const [name, origTrack] of this.humanoidTracks.rotation.entries()) {
      ...
      if (nodeName != null) {
        ...
        /* 
        （修正箇所）
        Mixamoの.fbxアニメーションデータから生成した.vrma（.glb）をVRMAnimation.tsを使って、
        動かしてみると報告通りチラつきが生じたので、以下の記事を参考にしてプログラムを加筆修正した。
        修正参考記事URL（https://github.com/pixiv/ChatVRM/issues/9）
        → 問題は解消された！！
        */
